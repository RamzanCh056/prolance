...
CREATE POLICY "Users view own usage" ON public.proposal_usage
  FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users insert own usage" ON public.proposal_usage
  FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users update own usage" ON public.proposal_usage
  FOR UPDATE USING (auth.uid() = user_id);

CREATE TRIGGER update_proposal_usage_updated_at
  BEFORE UPDATE ON public.proposal_usage
  FOR EACH ROW EXECUTE FUNCTION public.update_updated_at_column();
